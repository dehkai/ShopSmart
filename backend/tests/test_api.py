...
@patch("app.optimize")
def test_handle_basket_success(mock_optimize, mock_match_items, mock_fetch_db_items, mock_db_file, client):
    mock_db_file.exists.return_value = True
    mock_fetch_db_items.return_value = [(1, "Beras", "5kg")]
    
    # Mock MatcherResponse
    mock_matches = [
        ItemMatch(query="beras", item_code=1, item_name="Beras", confidence=0.9, resolved=True)
    ]
    ...
    mock_basket_result = BasketResult(
        ...
        items=[
            BasketItemResult(item_code=1, item_name="Beras", store_price=9.0)
        ],
        total=9.0,
        savings=1.0,
